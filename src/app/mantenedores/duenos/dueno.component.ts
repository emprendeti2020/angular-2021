...
import { Dueno, DuenosService, DuenoValidatorService } from 'src/app/comun/packs/dueno';
...
export class DuenoComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    duenos: Dueno[] = [];
    dataSource = new MatTableDataSource(this.duenos);
    totalCount: number = 0;

    @ViewChild(MatPaginator) paginator!: MatPaginator;

    private _unsubscribeAll: Subject<any> = new Subject<any>();
    private duenoSubscription: Subscription = new Subscription();
    ...
    ngOnInit(): void {
        this.getData();
        this.duenoSubscription = this.duenosService.duenosSubject.subscribe(() => {
            this.getData();
        });
    }

    ngOnDestroy(): void {
        this.duenoSubscription.unsubscribe();
    ...
    getData(pagprevia: number = 0, pag: number = 0, tam: number = 10): void {
        ...
        this.duenosService.getDuenos(pag, tam)
    ...
    openDialog(action: string, obj: any): void {
        obj.action = action;
        const dialogRef = this.dialog.open(DuenoDialogContent, {
            data: obj
        });
    ...
    agregarRowData(row_obj: Dueno): void {
        row_obj.estado = 0;

        this.duenosService.getKey("rut", row_obj.rut).
            subscribe(result => {
                if (result) {
                    this.eUtil.mostrarSnackbar('Dueño ' + row_obj.rut + ' ya existe, no se puede crear', 5500);
                }
                else {
                    this.duenosService.create(row_obj).subscribe(result => {
                        this.updateTabla();
                        this.eUtil.mostrarSnackbar('Dueño Creado');
                    })
                }
            });
    ...
    // tslint:disable-next-line - Disables all
    actualizarRowData(row_obj: Dueno): void {
        row_obj.estado = 0;
        this.duenosService.update(row_obj).subscribe(result => {
            this.updateTabla();
            this.eUtil.mostrarSnackbar('Dueño Actualizado');
    ...
    // tslint:disable-next-line - Disables all
    eliminarRowData(row_obj: Dueno): void {
        row_obj.estado = 0;
        this.duenosService.delete(row_obj).subscribe(result => {
            this.updateTabla();
            this.eUtil.mostrarSnackbar('Dueño Eliminado');
    ...
    exportToExcel(nTipo: number) {
        this.duenosService.getDuenos(-1)
            ...
            .subscribe(respuesta => {
                ...
                this.eUtil.exportXls(datos2, 'hoja1', 'Duenos');
                this.eUtil.mostrarSnackbar('Exportando Dueños...');
            });
...
export class DuenoDialogContent {
    action: string;
    // tslint:disable-next-line - Disables all
    local_data: any;
    selectedImage: any = '';
    ...
    constructor(
        private duenoValidatorService: DuenoValidatorService,
        private validatorService: ValidatorService,

        public datePipe: DatePipe,
        public dialogRef: MatDialogRef<DuenoDialogContent>,
        // @Optional() is used to prevent error if no data is passed
        @Optional() @Inject(MAT_DIALOG_DATA) public data: Dueno,
    ...
    doAction(): void {
        ...
        const dueno: Dueno = this.miFormulario.value;
        this.dialogRef.close({ event: this.action, data: dueno });
    }
    closeDialog(): void {
    ...
    createForm(): FormGroup {
        let form = this._formBuilder.group({
            id: ['', []],
            rut: ['', [Validators.required, this.validatorService.rutNovalido, Validators.pattern(this.validatorService.rutPattern)], [this.duenoValidatorService]],
            ...
            fono01: ['', [Validators.required, Validators.minLength(9), Validators.maxLength(13)]],
            fono02: ['', [Validators.minLength(9), Validators.maxLength(13)]],
            estado: ['', []],
            otro: ['', []]
        }, { updateOn: 'blur' });
        return form;
